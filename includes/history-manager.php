...
class WPPF_History_Manager {
	...
	public static function register_snapshot_post_type() {
		register_post_type(
			...
			array(
				'labels' => array(
					'name'          => __( 'Pin Snapshots', 'sectioncore-pin-freeze' ),
					'singular_name' => __( 'Pin Snapshot', 'sectioncore-pin-freeze' ),
				),
				'public'             => false,
	...
	public static function ajax_save_post_pin() {
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No tienes permisos para pinear este contenido.', 'sectioncore-pin-freeze' ),
				),
				403
			);
		}
		...
		wp_send_json_success(
			array(
				'message'     => $skip_snapshot ? __( 'Pin actualizado sin crear snapshot.', 'sectioncore-pin-freeze' ) : __( 'Pin guardado y snapshot creado.', 'sectioncore-pin-freeze' ),
				'post_id'     => $post_id,
				'is_pinned'   => $is_pinned,
				'html'        => $html,
	...
	public static function create_snapshot( $post_id, $html, $author_id ) {
		...
		if ( ! $post_id ) {
			return new WP_Error( 'invalid_post', __( 'No se pudo crear el snapshot: contenido inválido.', 'sectioncore-pin-freeze' ) );
		}

		$title = sprintf(
			/* translators: 1: post title, 2: date */
			__( 'Snapshot de %1$s - %2$s', 'sectioncore-pin-freeze' ),
