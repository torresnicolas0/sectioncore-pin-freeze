...
class WPPF_Ajax_Fetcher {
	...
	public static function ajax_fetch_frontend() {
		...
		if ( ! $post_id ) {
			wp_send_json_error(
				array(
					'message' => __( 'ID de contenido inválido.', 'sectioncore-pin-freeze' ),
				),
				400
			);
		}
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'No tienes permisos para capturar este contenido.', 'sectioncore-pin-freeze' ),
				),
				403
			);
		}
		...
		if ( ! $permalink ) {
			wp_send_json_error(
				array(
					'message' => __( 'No se pudo generar el permalink del contenido.', 'sectioncore-pin-freeze' ),
				),
				500
			);
		}
		...
		if ( '' === trim( $raw_html ) ) {
			wp_send_json_error(
				array(
					'message' => __( 'La respuesta del frontend llegó vacía.', 'sectioncore-pin-freeze' ),
				),
				500
			);
		}
	...
	private static function request_frontend_document( $permalink ) {
		...
		if ( $fallback_url ) {
			...
			$fallback_error = is_wp_error( $fallback_response )
				...
				: sprintf(
					...
					__( 'HTTP %d', 'sectioncore-pin-freeze' ),
					(int) wp_remote_retrieve_response_code( $fallback_response )
				);
		} else {
			$fallback_error = __( 'No se pudo construir URL de fallback local.', 'sectioncore-pin-freeze' );
		}
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'No se pudo capturar el frontend. Remoto: %1$s. Fallback local: %2$s.', 'sectioncore-pin-freeze' ),
				$primary_error,
				$fallback_error
			)
		);
	...
	private static function get_request_args() {
		return array(
			...
			'headers'     => array(
				...
				'User-Agent' => 'SectionCore Pin & Freeze/' . WPPF_VERSION,
			),
		);
	}
	...
	private static function extract_selector_html( $document_html, $selector_type, $selector_value ) {
		if ( ! class_exists( 'DOMDocument' ) ) {
			return new WP_Error( 'dom_missing', __( 'DOMDocument no está disponible en este servidor.', 'sectioncore-pin-freeze' ) );
		}
		...
		if ( '' === $selector_type || '' === $selector_value ) {
			return new WP_Error(
				'invalid_selector',
				__( 'Contrato de selector inválido: faltan tipo o valor.', 'sectioncore-pin-freeze' )
			);
		}
		...
		if ( '' === $xpath_query ) {
			return new WP_Error(
				'invalid_selector',
				__( 'El selector configurado es inválido para el tipo elegido (id/class/tag/xpath).', 'sectioncore-pin-freeze' )
			);
		}
		...
		if ( ! $loaded ) {
			return new WP_Error( 'parse_failed', __( 'No se pudo parsear el HTML del frontend.', 'sectioncore-pin-freeze' ) );
		}
		...
		if ( ! $nodes || 0 === $nodes->length ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'No se encontró contenido para el selector %1$s="%2$s".', 'sectioncore-pin-freeze' ),
		...
		$html = trim( implode( "\n", $chunks ) );
		if ( '' === $html ) {
			return new WP_Error( 'selector_empty', __( 'El selector existe, pero su contenido está vacío.', 'sectioncore-pin-freeze' ) );
		}
