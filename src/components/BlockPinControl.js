...
function BlockPinControl( {
	...
	const onRestoreHistory = ( historyIndex ) => {
		...
		if ( selectedEntry.html === ( pinnedHtml || '' ) ) {
			...
			setApplyNotice(
				__( 'Esta versión ya está aplicada.', 'sectioncore-pin-freeze' )
			);
			return;
		}
		...
		setDraftHtml( selectedEntry.html );
		setApplyNotice(
			__( 'Versión del historial restaurada y aplicada.', 'sectioncore-pin-freeze' )
		);
	};
	...
	return (
		<InspectorControls>
			<PanelBody title={ __( 'SectionCore Pin & Freeze', 'sectioncore-pin-freeze' ) } initialOpen>
				<ToggleControl
					label={ __( 'Pin HTML', 'sectioncore-pin-freeze' ) }
					help={ toggleHelp }
					checked={ isPinned }
				...
				{ isPinned && (
					<>
						...
						{ CodeEditor ? (
							...
							<TextareaControl
								label={ __( 'Frozen HTML', 'sectioncore-pin-freeze' ) }
								value={ draftHtml }
								onChange={ onChangeHtml }
								rows={ 12 }
						...
						<div className="wppf-html-actions">
							...
							>
								{ __( 'Aplicar cambios', 'sectioncore-pin-freeze' ) }
							</Button>
						</div>
