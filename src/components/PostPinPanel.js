...
async function requestWppfAjax( action, payload ) {
	...
	if ( ! ajaxUrl ) {
		throw new Error(
			__(
				'No se encontró la URL de AJAX para SectionCore Pin & Freeze.',
...
function formatSnapshotDate( dateValue ) {
	if ( ! dateValue ) {
		return __( 'Sin fecha', 'sectioncore-pin-freeze' );
	}
...
function getSnapshotAuthorName( snapshot ) {
	...
	return __( 'Autor desconocido', 'sectioncore-pin-freeze' );
}
...
function PostPinPanel() {
	...
	const onFetchLiveFrontend = async () => {
		...
		try {
			...
			setErrorMessage(
				error?.message ||
					__( 'Error al capturar frontend.', 'sectioncore-pin-freeze' )
			);
		} finally {
	...
	const onSavePin = async () => {
		...
		try {
			...
			if ( ! htmlToSave.trim() ) {
				throw new Error(
					__( 'No hay HTML para pinear.', 'sectioncore-pin-freeze' )
				);
			}
			...
			setSuccessMessage(
				data?.message ||
					__( 'Pin guardado correctamente.', 'sectioncore-pin-freeze' )
			);
		} catch ( error ) {
			setErrorMessage(
				error?.message ||
					__( 'Error guardando el pin.', 'sectioncore-pin-freeze' )
			);
		} finally {
	...
	const onRestoreSnapshot = async ( snapshot ) => {
		...
		try {
			...
			setErrorMessage(
				error?.message ||
					__( 'No se pudo restaurar el snapshot.', 'sectioncore-pin-freeze' )
			);
		} finally {
	...
	return (
		<PluginDocumentSettingPanel
			...
			title={ __( 'Post Pinning', 'sectioncore-pin-freeze' ) }
			className="wppf-post-panel"
		>
			<ToggleControl
				label={ __( 'Pin complete post HTML', 'sectioncore-pin-freeze' ) }
				help={
					isPinned
						? __(
			...
			<TabPanel
				...
				tabs={ [
					{
						name: 'capture',
						title: __( 'Capture', 'sectioncore-pin-freeze' ),
					},
					{
						name: 'history',
						title: __( 'Pin History', 'sectioncore-pin-freeze' ),
					},
				] }
			>
				{ ( tab ) => {
					...
					return (
						<>
							...
							<div className="wppf-post-actions">
								...
								>
									{ isSavingPin
										? __( 'Guardando…', 'sectioncore-pin-freeze' )
										: __(
												'Pin HTML (Guardar)',
												'sectioncore-pin-freeze'
										  ) }
