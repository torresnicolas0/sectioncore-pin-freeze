<?php
/**
 * Plugin Name:       SectionCore Pin & Freeze
 * Plugin URI:        https://github.com/torresnicolas0/sectioncore-pin-freeze
	...
	printf(
		...
		esc_html(
			sprintf(
				...
				__( 'SectionCore Pin & Freeze requiere WordPress %1$s o superior y PHP %2$s o superior. Versiones actuales: WordPress %3$s, PHP %4$s.', 'sectioncore-pin-freeze' ),
				WPPF_MIN_WP_VERSION,
...
class WPPF_Plugin {
	...
	public static function enqueue_editor_assets() {
		...
		wp_localize_script(
			...
			array(
				...
				'blockUnpinConfirm' => __( 'Este bloque está pineado. ¿Deseas despinearlo para editar?', 'sectioncore-pin-freeze' ),
				'postUnpinConfirm'  => __( 'Esta entrada está pineada. ¿Deseas despinearla para editar?', 'sectioncore-pin-freeze' ),
	...
	public static function display_post_states( $states, $post ) {
		...
		$states['wppf_pinned'] = sprintf(
			...
			esc_html__( 'Pineado', 'sectioncore-pin-freeze' )
		);
	...
	public static function page_row_actions( $actions, $post ) {
		...
		if ( self::is_post_pinned( $post->ID ) ) {
			$actions['wppf_unpin'] = sprintf(
				...
				esc_html__( 'Despinear', 'sectioncore-pin-freeze' )
			);
		} else {
			$actions['wppf_pin'] = sprintf(
				'<a class="wppf-row-action-link" href="%1$s">%2$s</a>',
				esc_url( self::get_pin_url( $post ) ),
				esc_html__( 'Pinear', 'sectioncore-pin-freeze' )
			);
		}
	...
	public static function handle_admin_actions() {
		...
		if ( ! wp_verify_nonce( $nonce, $nonce_action ) ) {
			wp_die( esc_html__( 'Nonce inválido.', 'sectioncore-pin-freeze' ) );
		}

		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			if ( 'pin_post' === $action ) {
				wp_die( esc_html__( 'No tienes permisos para pinear este contenido.', 'sectioncore-pin-freeze' ) );
			}

			wp_die( esc_html__( 'No tienes permisos para despinear este contenido.', 'sectioncore-pin-freeze' ) );
		}
	...
	public static function admin_notices() {
		...
		if ( 'post_pinned' === $notice ) {
			echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Contenido pineado correctamente.', 'sectioncore-pin-freeze' ) . '</p></div>';
			return;
		}

		echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Contenido despineado correctamente.', 'sectioncore-pin-freeze' ) . '</p></div>';
	}
