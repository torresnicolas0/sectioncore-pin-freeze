...
== Description ==
SectionCore Pin & Freeze lets administrators and editors freeze rendered HTML in two scopes:
...
== Installation ==
1. Upload the `sectioncore-pin-freeze` folder to `/wp-content/plugins/`.
...
== Upgrade Notice ==
= 1.0.4 =
Renamed plugin identity to SectionCore Pin & Freeze (`sectioncore-pin-freeze`) for WordPress.org review compliance.
...
== Changelog ==
= 1.0.4 =
* Renamed plugin display name to **SectionCore Pin & Freeze**.
* Updated slug/text domain from `pin-freeze` to `sectioncore-pin-freeze` across PHP, JS, readmes, and language assets.
* Renamed plugin bootstrap file/folder to match the new slug.
* Added explicit text-domain loading hook for the renamed domain.
